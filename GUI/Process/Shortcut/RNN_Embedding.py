...
description = '''
Constructs an RNN that uses an int sequence embedded as an input.
1) time_step: Set time size of input data.
2) id_size: Sets the value range of the int to be input.
3) embedding_size: Sets the size of the float vector to be created by embedding.
4) hidden_units: Sets the size of hidden vector.
5) hidden_Cell_func: Set the RNN cell to use.
6) hidden_state_reset: If True, the initial state at the beginning of the training is the last state of the previous learning. If False, initial state is used where all values are zero. Test does not apply the last state of previous learning (always 0 state).
7) output units: Sets the size of the output vector.
8) output_activation_func: Set activation function of hidden layer.
9) learning rate: Sets the learning rate.
'''
...
def Run_Shortcut(hNet, **parameters):
    ...
    hNet.process_Manager.RNN_Tensor_Generate(
        process_Name= parameters['process_Name'],
        name= 'RNN_Hidden',
        input_Tensor_Name= 'Embedded_Input',
